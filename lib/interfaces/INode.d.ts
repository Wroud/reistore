export declare type NodeValue<T> = T | ((value: T | undefined) => T | null) | null;
export declare type NodeArg = string | number | symbol | (string | number | symbol)[];
export declare type NodeArgsMap<TRoot> = Map<INode<TRoot, any, any, any, any>, any>;
export declare type GetNodeValue<T, TMultiple extends boolean> = TMultiple extends true ? (T | undefined)[] : (T | undefined);
...
export declare const PathNode: unique symbol;
...
export interface INode<TRoot, TModel, TValue, TParent extends INode<TRoot, any, TModel, any, any>, TMultiple extends boolean = false> extends IAccessor<TRoot, INode<TRoot, TModel, TValue, TParent, TMultiple>> {
    ...
    getFromNode<TR = TRoot>(object: TR, from: INode<TRoot, TR, any, any, any>, args?: NodeArgsMap<TRoot>): GetNodeValue<TValue, TMultiple>;
    get(object: TRoot, args?: NodeArgsMap<TRoot>): GetNodeValue<TValue, TMultiple>;
    setToNode<TR = TRoot>(object: TR, value: NodeValue<TValue>, to: INode<TRoot, TR, any, any, any>, args?: NodeArgsMap<TRoot>): IUndo<TRoot, this>;
    set(object: TRoot, value: NodeValue<TValue>, args?: NodeArgsMap<TRoot>): IUndo<TRoot, this>;
}
export declare type MarkMultiple<T> = {
    [P in keyof T]: T[P] extends INode<infer R, infer M, infer V, infer N, any> ? INode<R, M, V, N, true> : (T[P] extends object ? MarkMultiple<T[P]> : T[P]);
};
export interface ICountainer<T> {
    [PathNode]: T;
}
export declare type NodeContainer<TRoot, TParent, TModel, TNode extends INode<TRoot, any, TParent, any, any>> = ICountainer<INode<TRoot, TParent, TModel, TNode>>;
export interface IArgContainer<TRoot, TParent, TModel, TNode extends INode<TRoot, any, TParent, any, any>, TKey, TValue extends ICountainer<INode<TRoot, any, any, any, any>>> extends NodeContainer<TRoot, TParent, TModel, TNode> {
    <TA extends TKey | TKey[], TNode extends INode<TRoot, any, any, any, any> = TValue extends ICountainer<infer N> ? N : undefined>(arg?: TA, next?: (node: TA extends Array<any> ? MarkMultiple<TValue> : TValue) => ICountainer<TNode> | INodeAccessor<TRoot, TNode>): INodeAccessor<TRoot, TNode>;
}
export declare type FilteredKeys<T, U> = {
    [P in keyof T]: T[P] extends U ? P : never;
}[keyof T];
export declare type Part<T extends symbol | string | number, Value> = {
    [P in T]: Value;
};
export declare type AddKey<T, K extends string | number | symbol, V> = T & Part<K, V>;
export declare type Container<TRoot, TParent, TModel, TNode extends INode<TRoot, any, TParent, any, any>, TValue> = NodeContainer<TRoot, TModel, TValue, INode<TRoot, TParent, TModel, TNode>>;
export declare type ArgContainer<TRoot, TParent, TModel, TNode extends INode<TRoot, any, TParent, any, any>, TKey, TValue, TElement, TSchema> = IArgContainer<TRoot, TModel, TValue, INode<TRoot, TParent, TModel, TNode, any>, TKey, TSchema & Container<TRoot, TModel, TValue, INode<TRoot, TParent, TModel, TNode>, TElement>>;
export declare type ExtractNodeSetValue<TNode> = TNode extends INode<any, any, infer V, any, any> ? V : never;
export declare type ExtractNodeValue<TNode> = TNode extends INode<any, any, infer V, any, infer M> ? GetNodeValue<V, M> : never;
